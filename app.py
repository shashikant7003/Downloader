...
def download():
    ...
    # --- Yahan dhyan se samjho ---
    # Hum Render ki settings se cookies ka poora text utha rahe hain
    cookies_content = os.environ.get('YT_COOKIES', '')
    
    cookie_path = None
    # Agar Render par cookies milengi, toh code ek temporary file 'temp_cookies.txt' banayega
    if cookies_content:
    ...
    # Agar temporary cookies file ban gayi hai, toh use yt-dlp ko de do
    if cookie_path:
    ...
    try:
        ...
        return f"<body style='font-family:sans-serif; background:#f5f5f7; display:flex; justify-content:center; align-items:center; height:100vh;'><div style='background:#fff; padding:40px; border-radius:20px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); text-align:center;'><h2>❌ Error</h2><p>{str(e)}</p><br><a href='/'>Go Back</a></div></body>"
